...
void Samus::UpdateGateCastle(int dt)
{
...
void Samus::ChangeWeapon(EnumID idWeapon)
{
	//_weaponID = idWeapon;
}
...
void Samus::SetDirectDoor(EDirectDoor _direct)
{
	_directDoor = _direct;
	_colDoor = false;
}

EDirectDoor Samus::GetDirectDoor()
{
	if (_colDoor)
	{
		switch (_door->id)
...
bool Samus::AutoMove(int &range, int dt)
{
	if (range == 0)
		return true;
	if (range > 0)
...
void Samus::SimonDeath(int &_timeCount)
{
	if (_simonDeath)
	{
		_allowPress = false;
...
int Samus::GetHPSimon()
{
	return hp;
}


bool Samus::GetUsingCross()
{
	return _usingCross;
}

void Samus::SetUsingCross(bool value)
{
	/*if (!value)
	{
		_usingCross = false;
...
bool Samus::GetUsingWatch()
{
	return _usingWatch;
}

void Samus::SetUsingWatch(bool value)
{
	_usingWatch = value;
}